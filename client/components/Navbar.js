    ...
    <Menu as='div' className='fixed left-10 top-5 z-50'>
      ...
      >
        <Menu.Items className='absolute left-0 w-56 p-2 origin-top-right rounded-md bg-accent-950 shadow-lg ring-2 ring-primary-50/5 focus:outline-none'>
          {navbarItems.map(({ icon, title, link }) => (
            <div className='px-1 py-1' key={title}>
              <Menu.Item>
                {({ active }) => (
                  <Link
                    className={`${
                    ...
                    } group flex w-full items-center rounded-md px-3 py-2 text-sm z-50 hover:text-primary-50`}
                    to={link}
                  >
                    {icon}
                    {title}
